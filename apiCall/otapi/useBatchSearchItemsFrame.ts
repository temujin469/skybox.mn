  ...
  queryFn: async ({ queryKey: [primaryKey, variables] }) => {
    ...
    const xml = builder.buildObject({
      SearchItemsParameters:{
        CategoryId:variables.filters?.CategoryId
      }
    });
...
//     `;
    ...
    try {
      ...
      const uri = `https://otapi.net/service-json${primaryKey}?instanceKey=${key}&xmlParameters=${xml}&framePosition=${variables.start}&frameSize=${variables.limit}&blockList=Brand`;
