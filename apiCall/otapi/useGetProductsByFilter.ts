...
const useGetProductsByFilter = createQuery<Response, Variables>({
  primaryKey: "/FindCategoryItemSimpleInfoListFrame",
  queryFn: async ({ queryKey: [primaryKey, variables] }) => {
    try {
      const res = await otApi.get(
        `${primaryKey}?instanceKey=${key}&categoryId=${variables.catId}&categoryItemFilter=&framePosition=${variables.start}&frameSize=${variables.limit}`
      );
      return res.data;
    } catch (err) {
      console.log(err);
