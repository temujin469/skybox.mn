import React, { useState } from 'react';
import { Radio } from 'antd';
import { useRouter } from 'next/router';
import { Alert, AlertIcon, Box, Button, Heading, Input, Table, TableContainer, Text } from '@chakra-ui/react';
import useOrderDetail from '~/apiCall/strapi/useOrderDetail';
import { RootState } from '~/store/store';
import { useSelector } from 'react-redux';
import { formatCurrency } from '~/utilities/product-helper';

const ModulePaymentMethods = () => {
    const Router = useRouter();
    const [method, setMethod] = useState(1);
    const token = useSelector((state:RootState)=>state.auth.token);
    const router = useRouter();
    const orderId = router.query?.orderId;

    function handleChangeMethod(e:any) {
        setMethod(e.target.value); //e.target.value
    }

    function handleSubmit(e:any) {
        e.preventDefault();
        Router.push('/account/payment-success');
    }

    const {data} = useOrderDetail({variables:{jwt:token!,orderId:orderId as string}});

    const order = data?.data[0];

    const couponInput = (
      <div className='rounded-[5px] border my-10 p-3 w-full'>
        <p className='mb-4'>Хэрэв танд хөнгөлөлт урамшууллын купоны дугаар байгаа бол энд оруулна уу. Байхгүй бол алгасна уу.</p>
        <div className='flex items-center gap-2'>
          <Input placeholder="Кодоо оруулна уу" h="40px" className='border' />
          <Button h="40px">
            Идвэхжүүлэх
          </Button>
        </div>
      </div>
    )

    return (
      <>
        <Box
          borderRadius={5}
          border="1px"
          mb="40px"
          borderColor="gray.200"
          p={["13px", "20px"]}
        >
          <div className="ps-block__header">
            <Radio.Group onChange={(e) => handleChangeMethod(e)} value={method}>
              <Radio value={1}>
                <Heading>Qpay төлөх</Heading>
              </Radio>
              <Radio value={2}>
                <Heading>Дансаар төлөх</Heading>
              </Radio>
            </Radio.Group>
          </div>
          <div className="ps-block__content">
            {method === 1 ? (
              <Box
                display="flex"
                alignItems="center"
                flexDirection="column"
                justifyContent="center"
                pt="20px"
                pb={["0", "20px"]}
              >
                <img
                  style={{
                    maxWidth: "300px",
                  }}
                  src="data:image/png;base64,"
                />
                <div className="flex gap-4 mb-4">
                  <Heading size="sm" color="gray.500">
                    Нийт:
                  </Heading>
                  <Heading size="sm">{formatCurrency(order?.attributes.total_payment!)}₮</Heading>
                </div>
                <Text display={["none", "block"]} mb={3}>
                  Хэрэв та гар утаснаасаа орж байгаа бол ЭНД ДАРЖ банкны апп-аа сонгон төлбөрөө төлнө үү.
                </Text>
                <Alert status='warning' variant='left-accent'>
                  <AlertIcon />
                  Qpay гүйлгээний шимтгэл нийт үнийн дүгийн 1% болохыг анхаарна уу.
                </Alert>
                {couponInput}
                <Button size="lg" width="100%" variant="brand">
                  Төлөгдсөн эсэхийг шалгах
                </Button>
              </Box>
            ) : (
              <div className="ps-block__tab">
                <Box
                  display="flex"
                  alignItems="center"
                  flexDirection="column"
                  justifyContent="center"
                  pt="80px"
                  pb={["0", "20px"]}
                >
                  <div className="flex gap-4 mb-4">
                    <Heading size="sm" color="gray.500">
                      Банк / данс:
                    </Heading>
                    <Heading size="sm">Хаан банк,5779406497</Heading>
                  </div>
                  <div className="flex gap-4 mb-4">
                    <Heading size="sm" color="gray.500">
                      Төлбөр хүлээн авагч:
                    </Heading>
                      <Heading size="sm">Тэмүүжин</Heading>
                  </div>
                  <div className="flex gap-4 mb-4">
                    <Heading size="sm" color="gray.500">
                      Шилжүүлэх дүн:
                    </Heading>
                      <Heading size="sm">{formatCurrency(order?.attributes.total_payment!)}₮</Heading>
                  </div>
                    <div className="flex gap-4 mb-4">
                      <Heading size="sm" color="gray.500">
                        Гүйлгээний утга:
                      </Heading>
                      <Heading size="sm">{order?.attributes.order_id}</Heading>
                    </div>
                    <div>
                      <Alert status='warning' variant='left-accent' mt={10}>
                        <AlertIcon />
                        Та гүйлгээний утга дээр зөвхөн {order?.attributes.order_id} дугаарыг бичиж ХААН БАНК 5779406497 дансанд төлбөрөө шилжүүлнэ үү. Данс шилжүүлгээр хийсэн төлбөр 5 минутын дараа автоматаар баталгаажна.

                      </Alert>
                      {/* <Alert status='warning'>
                        <AlertIcon />
                        ORD эсвэл ОРД гэж заавал урд талд нь бичнэ. Дан тоог бичсэн тохиололд таны төлбөр автоматаар нэмэгдэхгүй болохыг анхаарна уу. АТМ-с төлбөр шилжүүлэхдээ гүйлгээний утгыг криллээр ОРД гэж бичиж болно.
                      </Alert> */}
                      {couponInput}
                    </div>
                </Box>
              </div>
            )}
          </div>
        </Box>
      </>
    );
};

export default ModulePaymentMethods;
